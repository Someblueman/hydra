...
`HYDRA_TEST_PLAN_REPAIR=combine HYDRA_TEST_PLAN_REUSE=1 HYDRA_TEST_PLAN_REPAIR_FAULT=1 sh tests/test_workflow_plan_task.sh` interrupts the per-step reset and resumes the same run and repair journal. Default and UBSan executions passed, including ordered first-ready, first-dispatch and terminal-observation timestamps for all four task steps. The original candidate, checks, input manifest and result bytes remain inspectable. `build/native-tests/test-plan-reuse-invalidation FIXTURE` checks a passing baseline and corrupted copies through the public result gate; it verifies canonical hash parity before recomputing a tampered repair digest.
