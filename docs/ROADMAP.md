...
before tmux removal; they do not wait for load balancing or distributed DAGs.
...
- [ ] **T3 — Distributed qualification and operator access.** Use T2 for priority
      5's two-host fan-out, validation, join, composition, and final check. Reuse
...
extend them to distributed runs as priority 5 lands.
...
- [ ] **V4 — Recovery visibility qualification.** Exercise the complete public CLI
      ...
      Repeat across two hosts with priority 5; one unavailable host cannot block
      observation of another, and stale results cannot advance dependent work.
