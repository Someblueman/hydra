...
- [ ] Conditional: extend the implemented [statistics view](STATISTICS.md) with queue delay,
...
The statistics extension remains conditional: current records do not provide
complete queue, run-duration, verification-time or recovery-history denominators.
Remote workflow cohorts require a future fleet protocol. These values remain
unavailable; this UI work does not create new telemetry or change that protocol.
