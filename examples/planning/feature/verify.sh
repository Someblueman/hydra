...
validator="$(shasum -a 256 "$0" | cut -d ' ' -f 1)"
...
record = {'obligation_id':'slug-check','subject_manifest_sha256':subject,'validator_identity':'feature-slug-v3','validator_recipe_sha256':recipe,'invocation':{'argv':['sh','verify.sh'],'exit_code':0},'environment':{'host':'local','toolchain':'cc/make/sh'},'case_inventory':['normalization','bounds','cli'],'observations':obs,'raw_evidence_sha256':hashlib.sha256(json.dumps(obs, sort_keys=True, separators=(',', ':')).encode()).hexdigest(),'counts':{'executed':1,'failed':0,'skipped':0},'limitations':['fixture-only']}
report = {'schema_version':3,'execution_status':'completed','evidence_status':'valid','domain_verdict':'pass','verdict':'pass','subject_sha256':subject,'validator_sha256':validator,'requirements':['normalization','bounds','cli'],'evidence':'sealed archive independently built and tested','limitations':['fixture-only'],'evidence_records':[record]}
