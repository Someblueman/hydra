#!/bin/sh
# Mixed agents demo: spawn multiple heads via --agents, show list/status and dashboard, then cleanup.
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
REPO_ROOT=$(CDPATH= cd -- "$SCRIPT_DIR/../.." && pwd)
TAPE="$SCRIPT_DIR/agents-demo.tape"

command -v vhs >/dev/null 2>&1 || { echo "vhs not installed" >&2; exit 1; }
command -v hydra >/dev/null 2>&1 || { echo "hydra not installed" >&2; exit 1; }

BASE="$(mktemp -d -t hydra-vhs-agents-XXXX)"
DEMO="$BASE/demo"
HY_HOME="$BASE/home"
FAKEBIN="$BASE/bin"
ZDOTDIR="$BASE/zdot"
...
rm -rf "$BASE"
echo "Agents demo done. GIF at $REPO_ROOT/assets/demos/agents-demo.gif"
