...
json_object *agent_run_cli(int argc, char **argv) {
    ...
    struct agent_stream stream = {0}; struct f_capture cap = {0}; struct f_control control = {0};
    bool preflight = argc == 4 && !strcmp(argv[0], "preflight"), invoked = false; int status = 1; size_t i, steering_bytes = 0;
    if (!preflight && (argc != 12 || strcmp(argv[0], "run"))) goto invalid;
    profile = agent_profile(argv[1]);
    if (!profile || !(prompt = prompt_read(argv[2])) || !requires(profile, argv[preflight ? 3 : 10]) || agent_profile_hash(profile, hash)) goto invalid;
    ...
    if (preflight) { result = f_success("agent-preflight", json_object_get(probe)); goto done; }
    if (!*argv[9] || strspn(argv[9], "0123456789") != strlen(argv[9]) || strtoul(argv[9], NULL, 10) > 86400 ||
        (strcmp(argv[11], "0") && strcmp(argv[11], "1"))) goto invalid;
    if (!getcwd(worktree, sizeof(worktree)) || !identity(argv[4], "project_") || !identity(argv[5], "head_") || !identity(argv[6], "instance_") ||
        snprintf(root, sizeof(root), "%s/state/v2/projects/%s/heads/%s", f_home, argv[4], argv[5]) >= (int)sizeof(root) ||
    ...
    args = agent_arguments(profile, *argv[7] ? "resume_argv" : "argv", values); if (!args) { unlink(path); goto invalid; }
    record = json_object_new_object(); json_object_object_add(record, "schema_version", json_object_new_int(1));
    f_string_add(record, "profile", argv[1]); f_string_add(record, "profile_sha256", hash); f_string_add(record, "instance_id", argv[6]);
    f_string_add(record, "project_id", argv[4]); f_string_add(record, "head_id", argv[5]); f_string_add(record, "worktree", worktree);
    ...
    if (task_write_json(argv[3], "agent.json", record, false)) { unlink(path); goto invalid; }
    {
        ...
        unsigned seconds = (unsigned)strtoul(argv[9], NULL, 10);
    ...
    if (!status && *argv[8]) {
        ...
        if (!answer || bytes > AGENT_OUTPUT_LIMIT || f_write(argv[8], answer, bytes, false)) status = 125;
    }
    if (!strcmp(argv[11], "1")) {
        char parent[F_PATH], *slash;
        if (f_copy(parent, sizeof(parent), argv[3]) || !(slash = strrchr(parent, '/'))) goto invalid;
    ...
    json_object_object_add(record, "usage", json_object_get(stream.usage));
    {
        ...
        json_object_object_add(record, "observed", observed);
    }
    if (*stream.session) f_string_add(record, "session_id", stream.session);
    json_object_object_add(record, "verification_passed", json_object_new_boolean(false));
    if (task_write_json(argv[3], "agent.json", record, true)) goto invalid;
