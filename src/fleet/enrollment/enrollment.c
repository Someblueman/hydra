...
static json_object *apply(const char *path, const char *confirm, unsigned seconds) {
    ...
    for (i = 0; i < json_object_array_length(hosts); i++) { char opid[256]; host = json_object_array_get_idx(hosts, i); if (!f_string(host, "operation_id")) { snprintf(opid, sizeof(opid), "%s:%s", saved, f_string(host, "alias")); f_string_add(host, "operation_id", opid); } previous = prior_row(progress, f_string(host, "alias")); if (previous) item = json_object_get(previous); else { item = json_object_new_object(); f_string_add(item, "alias", f_string(host, "alias")); f_string_add(item, "target", f_string(host, "target")); f_string_add(item, "operation_id", f_string(host, "operation_id")); f_string_add(item, "status", "pending"); json_object_array_add(rows, item); if (progress_file[0]) save_progress(progress_file, result); json_object_array_del_idx(rows, json_object_array_length(rows) - 1, 1); json_object_put(item); item = apply_host(host, seconds); } json_object_array_add(rows, item); if (progress_file[0]) save_progress(progress_file, result); }
