...
int plan_validation_write(const char *run, const char *step, const char *directory, const char *name) {
    json_object *compiled = plan_run_definition(run), *context = NULL; int status = -1;
...
enum plan_verdict plan_check_result(json_object *compiled, const char *run, json_object *check) {
    ...
    if (subject && file && (report = plan_read(path)) && plan_repair_fresh(run, check, f_string(subject, "sha256")))
        verdict = plan_report(compiled, check, report, f_string(subject, "sha256"));
...
int plan_step_check(const char *run, const char *step) {
    json_object *compiled = plan_run_definition(run), *observed = json_object_new_object(); int status = -1; char directory[F_PATH];
    ...
    if (plan_attempt_directory(run, step, directory) ||
