...
static bool strings(json_object *array, size_t minimum, bool ids) {
    ...
    for (i = 0; i < json_object_array_length(array); i++) {
        const char *s = task_text(json_object_array_get_idx(array, i));
        if (!plan_text(json_object_array_get_idx(array, i)) || (ids && !plan_id(s))) return false;
        for (j = 0; j < i; j++) if (!strcmp(s, task_text(json_object_array_get_idx(array, j)))) return false;
    }
    return true;
}
...
static bool permitted_scope(json_object *allowed, const char *s) {
    ...
    for (i = 0; i < json_object_array_length(allowed); i++) {
        const char *a = task_text(json_object_array_get_idx(allowed, i)); size_t n = strlen(a);
...
static bool envelope(json_object *o) {
    ...
    for (i = 0; i < json_object_array_length(f_field(o, "writes")); i++) if (!scope(task_text(json_object_array_get_idx(f_field(o, "writes"), i)))) return false;
    return true;
}
static bool recipe(json_object *step, json_object *env, json_object *errors, const char *path) {
    ...
    json_object *args = f_field(step, "args"), *argv = f_field(args, "argv"), *writes = f_field(step, "writes");
    ...
    if (!strcmp(kind, "spawn")) {
        ...
        if (argv) {
            ...
            for (i = 0; i < json_object_array_length(argv); i++) if (!yaml_text(task_text(json_object_array_get_idx(argv, i)))) {
            ...
            if (!plan_has(f_field(env, "tools"), task_text(json_object_array_get_idx(argv, 0)))) goto unauthorized;
        } else {
    ...
    for (i = 0; i < json_object_array_length(writes); i++) {
        const char *s = task_text(json_object_array_get_idx(writes, i));
...
int plan_validate(json_object *plan, json_object *policy, json_object *errors) {
    ...
    size_t i, j; int64_t seconds = 0; unsigned heads = 0; char path[128];
        ...
        plan_error(errors, "envelope.hosts", "unsupported_host", "only local execution is implemented");
    {
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (j = 0; j < json_object_array_length(set); j++) {
                const char *s = task_text(json_object_array_get_idx(set, j));
