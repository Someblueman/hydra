...
json_object *task_cli(int argc, char **argv) {
    ...
    if (!argc || !strcmp(argv[0], "help") || !strcmp(argv[0], "--help")) {
        ...
        f_string_add(data, "usage", "fleet task prepare --source DIR [--inputs-from DIR] --spec FILE --output PACKAGE; fleet task inspect --input PACKAGE; fleet task submit HOST --input PACKAGE --key KEY [--trust-spec HASH]; fleet task status HOST --id TASK_ID; fleet task observe HOST --id TASK_ID; fleet task start HOST --id TASK_ID --trust-spec HASH; fleet task resume HOST --id TASK_ID --trust-spec HASH; fleet task requests HOST --id TASK_ID; fleet task decide HOST --id TASK_ID --request REQUEST_ID --decision approve|reject --trust-spec HASH [--by LABEL]; fleet task result HOST --id TASK_ID [--output FILE] [--timeout SECONDS]; fleet task inspect-result --input FILE; fleet task collect HOST --id TASK --into DIR [--timeout SECONDS]; fleet task collect --input FILE --into DIR; fleet task collected --into DIR --id COLLECTION [--format candidates]; fleet task cancel HOST --id TASK_ID [--timeout SECONDS]; fleet task logs HOST --id TASK_ID [--source owner|work] [--stream stdout|stderr] [--offset N] [--limit N] [--step NAME --attempt N]");
