...
static int inputs(const char *directory, json_object *package) {
    ...
    for (i = 0; i < json_object_array_length(manifest); i++) {
        ...
        *strrchr(parent, '/') = '\0';
        if (f_mkdirs(parent) || f_hex_write(path, f_text(json_object_array_get_idx(payload, i)), 0600)) return -1;
    }
    return 0;
}
...
static int spawn_head(const char *directory, json_object *state, int64_t startup, struct f_control *control) {
    ...
    if (!evidence || !json_object_get_boolean(f_field(evidence, "ok")) || task_write_json(directory, "provenance.json", evidence, false)) goto done;
    if (!f_name(f_string(f_field(evidence, "data"), "head_id"))) goto done;
    ...
    json_object_put(evidence); f_capture_free(&cap); return status;
}
static void execution_argv(char **argv, json_object *work, json_object *state, bool command, bool resume) {
    size_t n = 0, i;
    argv[n++] = (char *)f_hydra;
    if (command) {
        ...
        for (i = 0; i < json_object_array_length(args); i++) argv[n++] = (char *)f_text(json_object_array_get_idx(args, i));
...
void task_execute(const char *directory, json_object *package, json_object *state, bool resume) {
    ...
    char *system = system_env ? strdup(system_env) : NULL;
    ...
    if (!resume && task_admission_wait(directory, state, &control)) goto failed;
    startup = monotonic() + json_object_get_int64(f_field(limits, "startup_seconds"));
    ...
    if (resume && task_admission_context(directory, state, spec)) { f_string_add(state, "failure", "admission_binding_invalid"); goto failed; }
    ...
    if (task_write_json(directory, "state.json", state, true)) goto done;
    execution_argv(argv, work, state, command, resume);
...
failed:
    ...
    if (f_string(state, "cancellation") && !strcmp(f_string(state, "cancellation"), "confirmed_stopped")) task_admission_children(state);
    else if (cap.timeout && !control.process.stop_unknown && task_no_agent_workers(state)) task_admission_children(state);
