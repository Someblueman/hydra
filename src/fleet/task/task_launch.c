...
bool task_runtime_valid(json_object *state) {
    ...
    const char *states[] = {"accepted", "starting", "running", "waiting_approval", "succeeded", "failed", "expired", "cancelled", "outcome_unknown", NULL}; size_t i;
    ...
    if (!strcmp(name, "waiting_approval") && (!f_name(f_string(state, "run_id")) || !f_number_is(state, "exit_status", 3) ||
        !f_string(state, "work_kind") || strcmp(f_string(state, "work_kind"), "workflow"))) return false;
...
static int detach(const char *directory, json_object *package, json_object *state, int owner, bool resume) {
    ...
    if (!child) {
        ...
        if (child) _exit(0);
        umask(077); task_execute(directory, package, state, resume); close(owner); _exit(0);
    }
...
static json_object *launch(const char *id, const char *trust, bool resume) {
    ...
    if (strcmp(f_string(state, "state"), resume ? "waiting_approval" : "accepted")) {
    ...
    if (!resume && !access(path, F_OK)) { result = f_error("fleet-task-start", "outcome_unknown", "a previous launch claim exists; inspect its recorded evidence without replaying work"); goto done; }
...
json_object *task_start(const char *id, const char *trust) { return launch(id, trust, false); }
json_object *task_resume(const char *id, const char *trust) { return launch(id, trust, true); }
