...
json_object *task_remote_cli(int argc, char **argv) {
    const char *host, *input = NULL, *key = NULL, *id = NULL; bool submit = !strcmp(argv[0], "submit");
    ...
    if (submit ? !input || !key : !id) return f_error("fleet-task", "invalid_input", "required task options are missing");
    ...
    if (!supported(response, submit ? "task-accept" : "task-status")) {
    ...
    f_string_add(request, "operation", submit ? "submit" : "status");
