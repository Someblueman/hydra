...
steps:
  ...
  - id: verify
    ...
    args:
      head: remote-agent
      name: prompt
      argv: [test, -s, received-prompt]
WORKFLOW
cat > "$fixture/source/.hydra/workflows/agents.json" <<'DATA'
{"schema_version":1,"inputs":{"first":{"source":"task","path":"agent-prompt","type":"file","max_bytes":1024},"followup":{"source":"task","path":"agent-followup","type":"file","max_bytes":1024}},"steps":{"first":{"inputs":{"prompt":{"input":"first"}},"outputs":{"answer":{"path":"answer.txt","type":"file","max_bytes":1024}}},"recall":{"inputs":{"followup":{"input":"followup"}},"outputs":{"recalled":{"path":"recalled.txt","type":"file","max_bytes":1024}}}}}
DATA
git -C "$fixture/source" add .hydra/workflows/agents.yml .hydra/workflows/agents.json
...
printf 'Remote agents: selected prompt, exact recorded-session recall, sealed artifacts, independent gate and collected metadata passed\n'
