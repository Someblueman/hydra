...
python3 - "$fixture/tmux-package" > "$fixture/terminal-request" <<'PYREQUEST'
import json, sys
print(json.dumps(dict(protocol=1, action='task', operation='submit',
                     package=json.load(open(sys.argv[1])), submission_key='terminal-direct')))
PYREQUEST
