...
def attached() -> None:
    with tempfile.TemporaryDirectory(prefix="hydra-attached-") as folder:
        ...
        try:
            ...
            s.send("(sleep 1; printf 'FORM_LIVE' > form-proof; printf 'FORM_LIVE\\n') &\r")
            s.pump(.2)
            s.send("\x02\t/")
            s.until("INPUT TO HYDRA / text field")
            s.pump(1.2)
            assert (one / "form-proof").read_text() == "FORM_LIVE"
            assert "FORM_LIVE" in s.screen.text(), "Form blocked attachment output"
            s.send("\x1b[200~search\nq\x1b[201~")
            s.until("searchq")
            assert "INPUT TO HYDRA / text field" in s.screen.text(), "Paste submitted a form command"
            s.send("\x1b[H界\x1b[C\x7f")
            s.until("界earchq")
            s.send("\x1b")
            s.pump(.2)
            assert "INPUT TO HYDRA / text field" not in s.screen.text()
            s.send("\t\t")
            s.until("INPUT TO AGENT")
            s.send("sleep 15\r")
            s.pump(.2)
