...
test_completion_includes_shipped_commands() {
    ...
    for cmd in spawn list switch kill regenerate state events status doctor dashboard dashboard-exit cycle-layout tui cleanup pr template completion version help group send recv tail broadcast wait-idle queue; do
        case "$bash_out" in
            *"$cmd"*)
