...
assert_contains "$ver_out" "Hydra version 2.2.1" "installed hydra reports 2.2.1"
...
sed 's/HYDRA_VERSION="2.2.1"/HYDRA_VERSION="2.2.0"/' "$prefix1/bin/hydra" > "$prefix1/bin/hydra.upgrade"
...
assert_contains "$upgrade_ver" "Hydra version 2.2.1" "upgrade replaces the older binary"
