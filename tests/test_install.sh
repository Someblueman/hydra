...
assert_contains "$ver_out" "Hydra version 2.6.0" "installed hydra reports 2.6.0"
...
sed 's/HYDRA_VERSION="2.6.0"/HYDRA_VERSION="2.4.0"/' "$prefix1/bin/hydra" > "$prefix1/bin/hydra.upgrade"
...
assert_contains "$upgrade_ver" "Hydra version 2.6.0" "upgrade replaces the older binary"
